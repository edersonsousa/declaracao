from tool_box import validar_nome_entry, validar_rg_entry, validar_cpf_entry, limpar_campos, toggle_check_a_partir, toggle_check_periodo_fechado, ato_box_select, lei_box_select, jornada_box_select, on_validate, validate_content, atualizar_declara, validar_tipo_de_servidor, declaracao
...
ato_combo.bind("<<ComboboxSelected>>", lambda event: 
        ato_box_select(event, ato_combo, a_partir_var, periodo_fechado_var, a_partir_checkbutton, periodo_fechado_checkbutton, lei_combo))
ato_combo.bind("<FocusOut>", lambda event: validar_tipo_de_servidor(ato_combo, cargo_de_origem_entry, bnt_n_servidor, bnt_servidor))
...
jornada_combo.bind("<<ComboboxSelected>>", lambda event:
        jornada_box_select(cargo_combo, lei_combo))

cargo_combo = Combobox(window, width=42, state="disable")

cargo_combo.grid(row=7, column=2)

ua_combo = Combobox(window, values=[
...
coordenadoria_combo = Combobox(window, 
                        values = [
...
cargo_de_origem_entry.bind("<FocusOut>", lambda event: validar_tipo_de_servidor(ato_combo, cargo_de_origem_entry, bnt_n_servidor, bnt_servidor))


regime_combo = Combobox(window, 
                        values = [
                                "Efetivo"       ,
                                "Lei 500"       ,
                                "Comissão"      ,
...
bnt_n_servidor = Button(text="Gerar Dados \n Não Servidor", width=15, bg="cyan", state="disable" , 
        command=lambda: declaracao(declara = {"Nome": nome_entry.get()}))
bnt_n_servidor["font"] = ("Montserrat","12")
bnt_n_servidor.grid(row=24, column=0, columnspan=2)

bnt_servidor = Button(text="Gerar Dados \n Servidor", width=15, bg="cyan", state="disable")
bnt_servidor["font"] = ("Montserrat","12")
bnt_servidor.grid(row=24, column=2, columnspan=2)

bnt_limpar = Button(text="Limpar", width=15, bg="gray", 
        command=lambda: limpar_campos(nome_entry, rg_entry, cpf_entry, estado_civil_combo, 
                                      ato_combo, jornada_combo, lei_combo, cargo_combo, destinacao_entry, 
                                      ua_combo, coordenadoria_combo, cargo_de_origem_entry, 
                                      a_partir_var, a_partir_checkbutton, periodo_fechado_var, periodo_fechado_checkbutton, regime_combo))
